...
public class FoodProvider extends BaseItemProvider {
    private List<Food> list;
    private AbilitySlice as;

    public FoodProvider(List<Food> list, AbilitySlice as) {
        this.list = list;
        this.as = as;
    }
    //返回item中要加载的布局对象
    ...
    public Component getComponent(int i, Component component, ComponentContainer componentContainer) {
        ...
        if (component!=null){
            ...
            //获取每一个item里面的布局对象
            dl = (DirectionalLayout) LayoutScatter.getInstance(as).parse(ResourceTable.Layout_food_item, null, false);
        }
        Food food = list.get(i);
        Text foodName = (Text)dl.findComponentById(ResourceTable.Id_food_name);
        Text foodPrice = (Text)dl.findComponentById(ResourceTable.Id_food_price);
