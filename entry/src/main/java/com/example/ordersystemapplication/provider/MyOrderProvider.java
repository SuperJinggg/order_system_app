...
public class MyOrderProvider extends BaseItemProvider {
    private List<Order> list;
    private AbilitySlice as;

    public MyOrderProvider(List<Order> list, AbilitySlice as) {
        this.list = list;
        this.as = as;
    }
    ...
    public Component getComponent(int i, Component component, ComponentContainer componentContainer) {
        ...
        if (component != null) {
            ...
            //获取每一个item里面的布局对象
            dl = (DirectionalLayout) LayoutScatter.getInstance(as).parse(ResourceTable.Layout_my_order_item, null, false);
        }
        Order order = list.get(i);
        Text order_id = (Text)dl.findComponentById(ResourceTable.Id_my_order_id);
